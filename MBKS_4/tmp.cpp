    ...
    for (int i = 0; i < N; i++)
    {
        if (Class->vsThThreads[i].wName[0] != 52428)
        {
            wcout << 
                ...
                Class->vsThThreads[i].stASLR.EnableBottomUpRandomization << L" " << Class->vsThThreads[i].stASLR.EnableForceRelocateImages << endl <<
                //L"ASLR:\t\t\t"         << ((Class->vsThThreads[i].bASLR) ? L"yes" : L"no") << endl <<
                endl;
        }
    }
    while (1) {}
